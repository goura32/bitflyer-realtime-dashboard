...
- executions パネル
  `raw_executions` から product_code ごとの直近価格レンジ、約定件数、合計サイズ、BUY/SELL 比率、フローバー、価格 sparkline を表示
- 板スナップショット表示
  `raw_board_snapshots` から asks / bids の上位レベルを常時表示
- 板差分表示
  `raw_board_deltas` から直近の asks / bids 変化レベルと mid / spread の変化量を表示
  size はヒート感のある強弱表示にしています
- alert パネル
...
- Executions
  product_code ごとの直近価格レンジ、約定件数、合計サイズ、BUY/SELL 件数、サイズ偏り、色付きフローバー、主導 side、価格ミニチャート
- Freshness
  `event_type x product_code` の鮮度を matrix で表示
- Board Snapshots
  各 product_code の asks / bids 上位レベルを terminal 上で常時表示
- Board Deltas
  各 product_code の直近 board delta で更新された価格帯、サイズ、mid / spread の変化量、方向感を表示
- Throughput
  系列別の件数と sparkline
- Latest Events
  直近イベントの一覧
...
- `Executions` の `Bias` が大きく片側へ寄る
  BUY/SELL のサイズ偏りが強く、板変化と合わせて短期フローの偏りを見やすくなります
- `Executions` の `Heat` が片側に寄る
  BUY 主導か SELL 主導かを数値を読まなくてもざっくり把握できます
...
- `Board Deltas` の size 表示が濃く偏る
  板変化のサイズ集中が一目でわかるので、どの価格帯に変化が寄っているか見やすくなります
...
現時点で、README に挙げていた主要な改善候補は反映済みです。

- board delta の方向感表示
- board delta の size 強弱表示
- executions bias の色付きバー表示
- executions の主導 side 強調

次に広げるとしたら、通知連携やより高度なチャート表現のような新機能フェーズです。
